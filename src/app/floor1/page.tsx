...
import { floor1Points } from "../lib/floor1points";
...
import { shortestPathFloor1 as shortestPath } from "../lib/floor1Graph";

// Only points with a visible name
const namedPoints = floor1Points.filter(
...
export default function Floor1Navigation() {
  ...
  const handleLocateMe = async () => {
    try {
      ...
      console.log("Local Wi‑Fi fingerprint (floor 1):", fpData);
      ...
      const res = await fetch("/api/locate?floor=1", {
        method: "POST",
  ...
  const { distanceMeters, estimatedTimeSeconds } = useMemo(() => {
    ...
    for (let i = 0; i < p.length - 1; i++) {
      const point1 = floor1Points.find((pt) => pt.id === p[i]);
      const point2 = floor1Points.find((pt) => pt.id === p[i + 1]);
  ...
  const pathPoints = useMemo(
    () =>
      visiblePath
        .map((id) => floor1Points.find((p) => p.id === id))
        .filter((p): p is NonNullable<typeof p> => Boolean(p))
        .map((p) => ({ fx: p.fx, fy: p.fy })),
    [visiblePath]
  );

  const startPoint = floor1Points.find((p) => p.id === startId);
  const endPoint = floor1Points.find((p) => p.id === endId);
            ...
            <div className="w-full h-full pt-6 pb-6 px-4 overflow-auto">
              <div className="inline-block">
                <RouteMap
                  ...
                  points={floor1Points}
                  backgroundImageUrl="/1stfloorfinal.svg"
                />
              </div>
            </div>
          </motion.div>

          {/* 3D Preview View (optional; can be blank for floor 1) */}
          <motion.div
            initial={{ opacity: 0 }}
