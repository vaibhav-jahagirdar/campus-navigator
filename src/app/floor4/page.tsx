...
import { floor4Points } from "../lib/floor4points";
...
import { shortestPathFloor4 as shortestPath } from "../lib/floor4Graph";

// Only points with a visible name
const namedPoints = floor4Points.filter((p) => p.tag && p.tag.trim().length > 0);

// Map and routing constants (adjust if 4th floor uses different scale)
...
export default function Floor4Navigation() {
  const [startId, setStartId] = useState<number>(1);
  const [endId, setEndId] = useState<number>(8);
  ...
  const handleLocateMe = async () => {
    try {
      ...
      console.log("Local Wi‑Fi fingerprint (floor 4):", fpData);
      ...
      const res = await fetch("/api/locate?floor=4", {
        method: "POST",
  ...
  const { distanceMeters, estimatedTimeSeconds } = useMemo(() => {
    ...
    for (let i = 0; i < p.length - 1; i++) {
      const point1 = floor4Points.find((pt) => pt.id === p[i]);
      const point2 = floor4Points.find((pt) => pt.id === p[i + 1]);
  ...
  const pathPoints = useMemo(
    () =>
      visiblePath
        .map((id) => floor4Points.find((p) => p.id === id))
        .filter((p): p is NonNullable<typeof p> => Boolean(p))
        .map((p) => ({ fx: p.fx, fy: p.fy })),
    [visiblePath]
  );

  const startPoint = floor4Points.find((p) => p.id === startId);
  const endPoint = floor4Points.find((p) => p.id === endId);
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }}>
              <motion.div variants={containerVariants} initial="hidden" animate="visible">
                ...
                <motion.div variants={itemVariants} className="mt-5">
                  <TurnByTurn path={visiblePath.length ? visiblePath : path} pathPoints={pathPoints} points={floor4Points} />
                </motion.div>
              </motion.div>
            </motion.div>
          )}
        ...
        <div className="flex-1 w-full overflow-auto relative">
            ...
            <div className="w-full h-full pt-6 pb-6 px-4 overflow-auto">
              <div className="inline-block">
                <RouteMap
                  ...
                  points={floor4Points}
                  backgroundImageUrl="/4thfloor.svg"
                />
              </div>
            </div>
          </motion.div>

          {/* 3D Preview View (optional) */}
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            <div className="w-full h-full flex items-center justify-center">
              <iframe
                src="https://3d.magicplan.app/#embed/?key=REPLACE_WITH_FLOOR4_EMBED_KEY"
                width="100%"
                height="100%"
                allowFullScreen
                className="border-0"
