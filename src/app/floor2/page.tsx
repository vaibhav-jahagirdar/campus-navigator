...
import { shortestPath } from "../lib/floor2Graph"
...
export default function Floor2Navigation() {
  const [startId, setStartId] = useState<number>(1)
  const [endId, setEndId] = useState<number>(21)
  ...
  const handleLocateMe = async () => {
    try {
      ...
      if (!fpRes.ok) {
        throw new Error(`Fingerprint helper error: HTTP ${fpRes.status} ${fpRes.statusText}`)
      }

      const fpData = (await fpRes.json()) as { aps?: Record<string, number> }
      console.log("Local Wi‑Fi fingerprint:", fpData)

      if (!fpData.aps || Object.keys(fpData.aps).length === 0) {
        alert("No Wi‑Fi data found from helper.\n" + "Make sure Wi‑Fi is ON and the IndoorLocator helper has access.")
        return
      }

      const res = await fetch("/api/locate?floor=2", {
        method: "POST",
      ...
      console.log("LocateMe result:", data)

      if (data?.estimatedPoint) {
        setLivePointId((prev) => (prev === data.estimatedPoint ? prev : data.estimatedPoint))
      } else {
        alert(data?.error || "Could not estimate location from Wi‑Fi fingerprint")
      }
    } catch (err) {
      console.error("LocateMe error:", err)
      alert("Error while estimating location.\n" + "Is the IndoorLocator helper running on this device?")
    } finally {
      setIsLocating(false)
    }
  }
  ...
  const { distanceMeters, estimatedTimeSeconds } = useMemo(() => {
    ...
    for (let i = 0; i < p.length - 1; i++) {
      const point1 = floor2Points.find((pt) => pt.id === p[i])
      const point2 = floor2Points.find((pt) => pt.id === p[i + 1])
  ...
  const pathPoints = useMemo(
    () =>
      visiblePath
        .map((id) => floor2Points.find((p) => p.id === id))
        .filter((p): p is NonNullable<typeof p> => Boolean(p))
        .map((p) => ({ fx: p.fx, fy: p.fy })),
    [visiblePath],
  )

  const startPoint = floor2Points.find((p) => p.id === startId)
  const endPoint = floor2Points.find((p) => p.id === endId)
    ...
    <div className="h-screen w-full flex gap-6 font-sfpro bg-slate-100 dark:from-slate-950 dark:to-slate-900 p-4">
      ...
      >
        ...
        >
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-sfpro font-semibold tracking-[-0.04em] text-slate-900 dark:text-white">Navigator</h1>
          </div>
          <p className="text-base text-slate-500 font-sfpro tracking-[-0.04em] font-medium  dark:text-slate-400 ml-12">Floor 2 Routing</p>
        </motion.div>
        ...
        >
          ...
          {path.length === 0 && (
            ...
            >
              <p className="text-sm">Select start and end points, then compute route</p>
            </motion.div>
          )}
        </motion.div>
      </motion.div>
      ...
      >
        ...
        >
          ...
          >
            <motion.div animate={{ rotate: show3D ? 180 : 0 }} transition={{ duration: 0.3, ease: "easeInOut" }}>
              <Cube size={18} />
            </motion.div>
            <span className="text-xs uppercase tracking-wide font-bold">{show3D ? "2D Map" : "3D View"}</span>
          </motion.button>
        </motion.div>
        ...
        <div className="flex-1 w-full overflow-auto relative">
          ...
          <motion.div
            ...
            className={`absolute inset-0 w-full h-full ${show3D ? "pointer-events-none" : "pointer-events-auto"}`}
          >
            <div className="w-full h-full pt-6 pb-6 px-4 overflow-auto">
              <div className="inline-block">
                <RouteMap
                  ...
                  points={floor2Points}
                  backgroundImageUrl="/2ndfloorfinal.svg"
                />
              </div>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            transition={{ duration: 0.4, ease: "easeInOut" }}
            className={`absolute inset-0 w-full h-full ${show3D ? "pointer-events-auto" : "pointer-events-none"}`}
          >
            <div className="w-full h-full flex items-center justify-center">
              <iframe
                src="https://3d.magicplan.app/#embed/?key=MDc0ZDVjMWFhZmJkMmE1MzRjNzZhMTY4MTRhOTVkYTM1ZmJhOTc4NDU0ZDNhODUzNWQzNTFjYTQ2ZWJjMzhkNO7DR22mLdZAkIASEhTon0GlFMTddTpNXfZa17DtezYJBoUwFxid5Dntwkzc1FR%2Ba0Ea32AoUjx2jKhny8f9zPNVO9z2NRfVkDoK3kMfaeKU"
                width="100%"
                height="100%"
                allowFullScreen
                className="border-0"
